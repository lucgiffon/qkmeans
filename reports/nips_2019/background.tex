...
if $\rmS_q \in \mathcal{E}_q$ and $\delta_{\mathcal{E}_q}(\rmS_q)=+\infty$ otherwise, $\mathcal{E}_q$ being a constraint set that typically impose a sparsity structure on its elements, as well as some other contraint such as unit Frobenius norm (see~\ref{}\todo{add some details about scaling and $\lambda$ somewhere, maybe in appendix with the palm algorithm}). \addLG{Oui, plutôt dans l'appendice. Ca ne me semble pas utile d'en parler ici car on peut considérer que $S_0 = \lambda$ et donc cette formulation est tout à fait correcte sans être alourdie}
...
ascertained, the authors of \cite{bolte2014proximal} derived an algorithm called \textit{Hierarchical PALM4MSA} that is able to find good local minima and that has
a version that is provably convergent. \addLG{j'ai l'impression qu'il y a contre-sens là. Hiearchical PALM4MSA n'est pas garanti de converger}
