...
\caption{Clustering Blobs data: running times of the assignation step, averaged over \addVE{?} runs, for batches of $\nexamples=5000$ examples in dimension $\datadim=2000$. The vertical black lines are the standard deviation w.r.t. the runs and the average number of parameters actually learned  in the models are reported above those lines.\addVE{to be completed}.}
...
\caption{Nearest neighbor search on blobs data. Results are averaged over ??runs (vertical lines: standard deviation) and the average number of parameters actually learned is reported above each bar. \addVE{to be completed: how many runs?}.}
\label{fig:nn:blobs}
\end{figure}
