...
\todo[inline]{Figures~\ref{fig:mnist:objfun} and~\ref{fig:fmnist:objfun} to be updated: remove curves for sparsity 1 ; remove title; update legend text as ``Hierarch. QKmeans, sparsity level 3''; legend inside; y axis named ``Objective function''; larger font size.}
...
\caption{Clustering Blobs data: running times of the assignation step, averaged over \addVE{?} runs, for batches of $\nexamples=5000$ examples in dimension $\datadim=2000$. The vertical black lines are the standard deviation w.r.t. the runs and the average number of actual parameters in the models are reported above those lines.\addVE{to be completed}.}
