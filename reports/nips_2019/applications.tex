...
1NN KD Tree                         & N/A   \\
1NN Ball Tree                       & N/A   \\ \midrule
1NN K-means 128 Clusters            &       \\
1NN K-means 256 Clusters            &       \\
1NN K-means 512 Clusters            &       \\ \midrule
1NN QK-means 128 Clusters           &       \\
1NN QK-means 256 Clusters           &       \\
1NN QK-means 512 Clusters           &       \\ \midrule \midrule
Nyström K-means + SVM 128 Clusters  &       \\
Nyström K-means + SVM 256 Clusters  &       \\
Nyström K-means + SVM 512 Clusters  &       \\ \midrule
Nyström QK-means + SVM 128 Clusters &       \\
Nyström QK-means + SVM 256 Clusters &       \\
Nyström QK-means + SVM 512 Clusters &       \\ \bottomrule
\end{tabular}

\caption{Results on the classification task on \texttt{Blobs} dataset. Results are averaged over 5 runs. ``N/A'' denotes experiments that did not finish.}
...
% Nyström K-means + SVM 10 Clusters  &       \\
% Nyström K-means + SVM 16 Clusters  &       \\
% Nyström K-means + SVM 30 Clusters  &       \\ \midrule
% Nyström QK-means + SVM 10 Clusters &       \\
% Nyström QK-means + SVM 16 Clusters &       \\
% Nyström QK-means + SVM 30 Clusters &       \\ \bottomrule
\end{tabular}
...
\end{table}
\todo{Ajouter les résultats Nyström + SVM aux jeux de données MNIST et Fashion Mnist}
...
Our algorithm could allow one to obtain an efficient Nyström approximation, while not reducing too much the quality of the K-means landmark points which are expressed as a factorization of sparse matrix. 
