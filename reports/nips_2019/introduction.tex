...
Beyond its popularity for clustering, K-means algorithm~\cite{hartigan1979algorithm,jain2010data} is a pivotal procedure for other core machine learning and data analysis techniques such as indexing~\cite{addref}, nearest-neightbors prediction~\cite{addref}, as well as for more specific approaches like the Nyström approximation for kernel machines~\cite{si2016computationally}.
%
As data dimensionality and data sample size continue to grow, it is critical to produce viable and cost-effective alternatives to the computationally expensive usual K-means algorithm. Previous attempts to alleviate the computational issues in K-means often relied on sparsity- and randomization-based methods~\cite{boutsidis2014randomized,shen2017compressed,liu2017sparse}.
...
%
These works primarily focused on fast transforms such as Fourier and Hadamard transforms, which are fixed before the learning begins. An interesting question is whether one can go beyond that and learn the fast transform from data. \addHK{In a recent paper, \ldots, parler du papier de Gribonval \ldots In this paper, we take this idea further and provide ...}
%
