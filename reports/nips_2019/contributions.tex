...
Building upon~\eqref{eq:kmean_problem} and~\eqref{eq:palm4msa} the optimization problem 
of \qkmeans may be written as follows:
%
\begin{align}
\label{eq:qmean_problem}
 \argmin_{\rmS_1, \ldots, \rmS_{\nfactors}, \rvt} g\left(\rmS_1, \ldots, \rmS_{\nfactors}, \rvt\right)
    \eqdef \sum_{k\in\intint{\nclusters}}\sum_{j: t_j = k} \left\|\rvx_j -\rvv_k\right\|^2 + \sum_{q\in\intint{\nfactors}} \delta_{\mathcal{E}_q}(\rmS_q) \text{ s. t. } \rmV = \prod_{\in\intint{\nfactors}}\rmS_q
\end{align}
%
Here, 
...
\begin{align*}
	\sum_{j: t_j = k} &\left\|\rvx_j -\rvv_k\right\|^2
	=\sum_{j: t_j = k} \left\|\rvx_j -\rvu_k+\rvu_k-\rvv_k\right\|^2\\
	& = \sum_{j: t_j = k}\left(\|\rvx_j-\rvu_k\|^2+\|\rvu_k-\rvv_k\|^2-2\langle\rvx_j-\rvu_k,\rvu_k-\rvv_k\rangle\right)\notag\\
	& = \sum_{j: t_j = k}\|\rvx_j-\rvu_k\|^2+n_k\|\rvu_k-\rvv_k\|^2- 2\sum_{k=1}^{K}\left\langle\underbrace{\sum_{j: t_j = k}(\rvx_j-\rvu_k)}_{=0},\rvu_k-\rvv_k\right\rangle\notag\\
%	&= \sum_{k=1}^{K}\left[\sum_{j: \rvt_j = k}\|\rvx_j-\rvu_k\|^2+n_k\|\rvu_k-\rvv_k\|^2-\langle n_k\rvu_k-n_k\rvu_k,\rvu_k-\rvv_k\rangle\right]\\
	&= \sum_{j: t_j = k}\|\rvx_j-\rvu_k\|^2 + n_k\|\rvu_k-\rvv_k\|^2
\end{align*}
...
of this section.}
\todo[inline]{In algorithm~\ref{algo:qmeans}, shall we remove ${(\tau)}$ to have simpler notation?}
\todo[inline]{Shall we simplify algorithm~\ref{algo:qmeans}, and add subroutines like centroid(.), fast-operator update (.), fast-assignement(.)?}
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\STATE $\forall k\in\intint{\nclusters}, \rvu_k \leftarrow \frac{1}{n_k^{(\tau)}} \sum_{n: t_n^{(\tau)}= k} {\rvx_n}$
with $n_k^{(\tau)} \eqdef |\{n: t_n^{(\tau)}=k\}|$
		\COMMENT{$\bigO{\nexamples\datadim}$}
...
%		\label{line:qmeans:endkmeans}
		\STATE $\rmA \leftarrow \rmD_{\sqrt{\rvn^{(\tau)}}} \times \rmU^{(\tau)} $
		\COMMENT{$\bigO{\nclusters\datadim}$}
		\label{line:qmeans:A}
		...
		\label{line:qmeans:S}
		\STATE $\rmS_1^{(\tau)} \leftarrow \rmD_{\sqrt{\rvn^{(\tau)}}}^{-1} \times \rmS_1^{(\tau)}$
		\COMMENT{$\bigO{AB}$}
		\label{line:qmeans:normalizeS}
		\STATE $\rmV^{(\tau)} \leftarrow \prod_{q\in\intint{\nfactors}}{\rmS_q^{(\tau)}}$
...
\begin{split}
 ...
 = \argmin_{\{ \mathcal{S}_1, \ldots,\mathcal{S}_Q\}, \lambda} & ||\rmD_{\sqrt{\rvn^{(\tau)}}}~(\hat{\rmX}^{(\tau)} - \rmU)||_{\mathcal{F}} ^ 2  \\
...
\end{equation*}
%
with $\mathcal{S}_0$ fixed and equal to $\rmD_{\sqrt{\rvn^{(\tau)}}}$. The Equation \ref{eq:qmeans_problem_t_fixed} can then be rewritten as
%
\begin{equation}
\begin{split}
...
%\ENDFOR
%\STATE $\hat \rmU \eqdef \prod_{j=1}^{Q} \mathcal{S}_j^{i+1}$
