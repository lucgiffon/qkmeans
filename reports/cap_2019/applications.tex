...
Standard kernel machines are often prohibited in large scale applications because of their associated Kernel matrix $\rmK$ which has $O(n^2)$ storage and $O(n^2d)$ computational complexity: $\forall i,j \in [\![n]\!], \rmK_{i,j} = k(\rmX_i, \rmX_j)$. A well-known strategy to overcome this problem is to use the Nyström method which compute a low-rank approximation of the Kernel matrix on the basis of some pre-selected landmark points. 
...
Our algorithm could allow to get an efficient Nyström approximation while keeping the quality of the K-means landmark points, expressed as a factorization of sparse matrix.  
