...
%\end{align}
where the matrix of centers is parameterized as
\begin{equation}
	\label{eq:centers}
...
%
This is a \todo{(???)} regularized \todo{()???)}  version of the \kmeans optimization problem~\eqref{eq:kmean_problem} in which centers $\rvv_k$ are constrained to form a matrix $\rmV$ with a fast-operator structure, the indicator functions $\delta_{\mathcal{E}_q}$ imposing the sparsity of matrices $\rmS_q$.
...
\begin{proof}
We show that each of the assignment and center update steps of the algorithm does not increase the overall objective. 
...
\todo[inline]{Ajouter le déroulement de l'equation centers update step dans les supplementary materials}
...
Note that this cost is dominated by the cost of the assignement step as soon as the number of examples $\nexamples$ is greater than $\log^3 A$. \todo[inline]{Faire un supplementary material montrant les expériences préliminaires de gain potentiel de temps lors de l'entrainement}
