...
which is a sufficient condition to assert the sequence of $g$ converges. \palm can be designed so that it returns its initialization if it didn't manage to lower the objective. Hence the sequence of $\rmV$ also converges, so does Algorithm~\ref{algo:qmeans}.
