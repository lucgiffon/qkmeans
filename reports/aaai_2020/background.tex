...
solve the \kmeans problem is Lloyd's algorithm, which alternates between
...
The core feature of the related fast algorithms is that the matrix $\rmU\in\sR^{M\times M}$ of such linear operators can be written as the product $\rmU=\Pi_{q\in\intint{\nfactors}}\rmS_q$ of $\nfactors=\bigO\left (\log M\right )$ sparse matrices $\rmS_q$ and $\left \|\rmS_q\right \|_0=\mathcal{O}\left (M\right )$ non-zero coefficients per factor \cite{LeMagoarou2016Flexible,Morgenstern1975Linear}:
