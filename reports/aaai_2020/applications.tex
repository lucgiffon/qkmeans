...
\end{tabular}}
\caption{
Nystr\"om approximation results: reconstruction error and running times for landmark selection methods based on either \kmeans, \qkmeans or uniform sampling. The \qkmeans results are obtained with sparse factors with at least 2 values in each line and each column. The uniform sampling based Nyström approximation running times are not displayed because they are the same as for the Nyström approximation based on \kmeans centers. Every experiment results are averaged over 5 runs. The standard deviation for the reconstruction error are not displayed because they are lower than $0.005$. Best results are bold while second best are underlined.}
\label{tab:nystrom}
\end{table*}
