...
if __name__ == "__main__":
    ...
    lst_input_path = [
        ...
        "2020/10/9_10_kmeans_proj_l1",
        "2020/10/9_10_kmeans_proj_l1_bis"
        # "2020/10/8_9_kmeans_proj_l1",
        # "2020/10/8_9_kmeans_proj_l1_coil_kddcup09"
    ...
    for i, row in df_results.iterrows():
        ...
        dct_results["L1-proj"].append(bool(row["--l1-proj"]) if not np.isnan(row["--l1-proj"]) else False)
        dct_results["--lambda-l1-proj"].append(float(row["--lambda-l1-proj"]) if not (row["--lambda-l1-proj"] == "None" or np.isnan(row["--lambda-l1-proj"])) else None)
        dct_results["actual_param_lambda"].append(float(row["actual_param_lambda"]) if not np.isnan(row["actual_param_lambda"]) else None)
        dct_results["--epsilon-tol-proj"].append(float(row["--epsilon-tol-proj"]) if not np.isnan(row["--epsilon-tol-proj"]) else None)
