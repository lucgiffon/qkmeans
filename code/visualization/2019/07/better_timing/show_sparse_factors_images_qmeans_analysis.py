...
if __name__ == "__main__":
    suf_path = "2019/07/qmeans_analysis_better_timing"
    input_dir = "/home/luc/PycharmProjects/qalm_qmeans/results/" + suf_path
    output_dir = "/home/luc/PycharmProjects/qalm_qmeans/reports/figures/" + suf_path + "/sparse_factors"
    ...
    df_results_kmeans_palm = df_results_kmeans[df_results_kmeans["palm"]]
    ...
    for dataset_name in datasets:
        ...
        # first deal with qmeans results
        ################################
        df_results_kmeans_palm = df_results_kmeans_palm[df_results_kmeans_palm[datasets_col]]
        nb_factors = [min(int(np.log2(nb_cluster)), int(np.log2(dataset_dim[dataset_name]))) for nb_cluster in lst_nb_cluster_values]
        for hierarchical_value in [True, False]:
            df_hierarchical = df_results_kmeans_palm[df_results_kmeans_palm["--hierarchical"] == hierarchical_value]
            ...
            for idx_sparsy_val, sparsy_val in enumerate(lst_sparsity_values):
                ...
                for idx_nb_clust, clust_nbr in enumerate(lst_nb_cluster_values):
                    ...
                    for oar_id in df_nb_clust["oar_id"]:
                        ...
                        sparse_factors = dct_centroids[oar_id]
                        # lst_objectives_seeds.append(flat_centroids)
                        show_sparse_factors(sparse_factors, "{} Kmeans palm {} clusters sparsity factor {} {}{}".format(dataset_name, clust_nbr, sparsy_val, oar_id.split(".")[-1], " hierarchical" if hierarchical_value else ""))
