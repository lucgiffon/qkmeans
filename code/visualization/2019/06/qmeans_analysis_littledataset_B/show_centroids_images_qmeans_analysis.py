...
if __name__ == "__main__":
    input_dir = "/home/luc/PycharmProjects/qalm_qmeans/results/2019/06/qmeans_analysis_littledataset_B"
    output_dir = "/home/luc/PycharmProjects/qalm_qmeans/reports/figures/2019/06/qmeans_analysis_littledataset_B/centroids_display"
    ...
    for dataset_name in datasets:
            ...
            for idx_sparsy_val, sparsy_val in enumerate(lst_sparsity_values):
                ...
                for idx_nb_clust, clust_nbr in enumerate(lst_nb_cluster_values):
                    ...
                    for oar_id in df_nb_clust["oar_id"]:
                        ...
                        show_centroids_from_vector(flat_centroids, "{} Qmeans {} clusters sparsity factor {} {}{} - nb param {}".format(dataset_name, clust_nbr, sparsy_val, oar_id.split(".")[-1], " hierarchical" if hierarchical_value else "", nbr_param), shapes[dataset_name])
        ...
        for idx_nb_clust, clust_nbr in enumerate(lst_nb_cluster_values):
            ...
            for oar_id in df_nb_clust["oar_id"]:
                ...
                # lst_objectives_seeds.append(flat_centroids)
                show_centroids_from_vector(flat_centroids, "{} Kmeans {} clusters {} - nb param {}".format(dataset_name, clust_nbr, oar_id.split(".")[-1], nbr_param), shapes[dataset_name])
        ...
        # fig, ax = plt.subplots()
        for idx_sparsy_val, sparsy_val in enumerate(lst_sparsity_values):
            df_sparsy_val = df_dataset_kmeans_palm[df_dataset_kmeans_palm["--sparsity-factor"] == sparsy_val]

            for idx_nb_clust, clust_nbr in enumerate(lst_nb_cluster_values):
                df_nb_clust = df_sparsy_val[df_sparsy_val["--nb-cluster"] == clust_nbr]
                df_nb_clust = df_nb_clust[df_nb_clust["--hierarchical"]]
                lst_centroids_seeds = []
                for oar_id in df_nb_clust["oar_id"]:
                    # get centroid matrix for each seed (refered by the oar id)
                    flat_centroids = dct_centroids[oar_id].compute_product(return_array=True)
                    nbr_param = dct_centroids[oar_id].get_nb_param()
                    # lst_objectives_seeds.append(flat_centroids)
                    show_centroids_from_vector(flat_centroids, "{} Kmeans with palm {} clusters {} sparsity factor {} - nbr param {}".format(dataset_name, clust_nbr, oar_id.split(".")[-1], sparsy_val, nbr_param), shapes[dataset_name])
