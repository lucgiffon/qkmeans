...
if __name__ == "__main__":
    ...
    suf_path = "2019/08/3_4_qmeans_analysis_blobs_hierarchical_or_not_30000"
    suf_path_fail = "2019/08/3_4_qmeans_analysis_blobs_hierarchical_or_not_30000_only_fail"
    input_dir = create_input_dir(suf_path)
    input_dir_fail = create_input_dir(suf_path_fail)


    output_dir = "/home/luc/PycharmProjects/qalm_qmeans/reports/figures/"+ "2019/08/aaai_conference" + "/hierar_or_not_3000/histogrammes"
    output_dir = pathlib.Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)


    df_results = get_df(input_dir)
    df_results_fail = get_df(input_dir_fail)

    df_results = pd.concat([df_results, df_results_fail])
