    ...
    for data in datasets:
        ...
        for task in tasks:
            ...
            for init in init_schemes:
                ...
                set_lambda_values = sorted(set(df_kmeans_l1_proj["actual_param_lambda"].values))
                max_compression_rate = 1
                # for lambda_val in set_lambda_values:
                lambda_val = dct_data_correct_lambda[data]
                df_lambda = df_kmeans_l1_proj[df_kmeans_l1_proj["actual_param_lambda"] == lambda_val]
                ...
                trace_name = "Kmeans L1 proj {}".format(float(lambda_val))
                print(trace_name, task_values_mean)
                # if task == "compression-rate":
