...
from qkmeans.palm.palm_fast import palm4msa_fast1, palm4msa_fast2, \
...
class TestPalm4Msa(unittest.TestCase):
    ...
    def test_palm4msa_compare34(self):
        for k, X in self.data.items():
            ...
            np.testing.assert_almost_equal(out4[0], out3[0],
                                           err_msg='f_lambda')
            self.assertEqual(out4[1].n_factors, nb_factors, msg='nb factors')
            for j in range(nb_factors):
                np.testing.assert_array_almost_equal(
                    out4[1].get_factor(j).toarray(),
                    out3[1].get_factor(j).toarray(),
                    err_msg='Factor {}'.format(j))
            np.testing.assert_array_almost_equal(out4[2], out3[2],
                                                 err_msg='arr_X_curr')
            np.testing.assert_equal(out4[4], out3[4], err_msg='i_iter')
            ...
            np.testing.assert_array_almost_equal(out4[2], out3[2],
                                                 err_msg='arr_X_curr')
            np.testing.assert_equal(out4[4], out3[4], err_msg='i_iter')
            np.testing.assert_array_almost_equal(out4[3], out3[3],
                                                 err_msg='objective_function')
