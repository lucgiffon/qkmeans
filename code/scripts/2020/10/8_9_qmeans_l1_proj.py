...
Options:
  ...
  --no-1nn-baselines                    Doesn't run 1nn baselines.

Kmeans L1 proj:
  --l1-proj                             Make the L1 projection on the L1 ball
  --lambda-l1-proj float                Radius of the L1 ball for the projection of the centroids
  --epsilon-tol-proj float              Epsilon value for the tol on the L1 proj.
...
Dataset:
  ...
  --coil20 int                          Use coil20 dataset with sqaure iamges of size int.
...
def main_kmeans(X, U_init):
    ...
    if paraman["--minibatch"]:
        objective_values_k, final_centroids, indicator_vector_final = kmeans_minibatch(X_data=X,
                                                                                       ...
                                                                                       _lambda=paraman["--lambda-l1-proj"],
...
def make_1nn_evaluation(x_train, y_train, x_test, y_test, U_centroids, indicator_vector):
    ...
    #
    if paraman["kmeans"] and not paraman["palm"] and paraman["--minibatch"] is None and not paraman["--no-1nn-baselines"]:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if paraman["kmeans"]:
            ...
            elif paraman["--l1-proj"]:
                dct_nb_param = {"nb_param_centroids": np.sum(U_final.astype(bool).astype(int))}
